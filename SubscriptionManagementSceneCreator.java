...
public class SubscriptionManagementSceneCreator extends SceneCreator implements EventHandler<MouseEvent>  {
	...
	public SubscriptionManagementSceneCreator(double width, double height) {
        ...
        TableColumn<Subscription, Double> totalCostColumn= new TableColumn<>("Συνολικό Κόστος");
        totalCostColumn.setCellValueFactory(new PropertyValueFactory<>("totalCost"));
        subscriptionTableView.getColumns().add(totalCostColumn);
	...
	public void handle(MouseEvent event) {
        ...
        if (event.getSource() == newSubBtn) {
		  ...
		  /*  if (selectedAthlete == null || trainingProgram == null) {
		        Alert alertType = new Alert(Alert.AlertType.ERROR);
		        alertType.setTitle("Missing Information");
		        alertType.setContentText("Please select an athlete and a training program.");
		        alertType.show();
		        return;
		    }*/
		    ...
		    calculateTotalCost();
		    Enrollment enrollment = AthleteManagementSceneCreator.enrollment; 
		    
		    createPayment(payCode, payDate, paymentMethod, selectedSubscription, enrollment, totalCost);
	...
	private void createPayment(int payCode, String payDate, String paymentMethod, Subscription selectedSubscription,
			Enrollment enrollment, double totalCost) {
		Payment payment= new Payment(payCode, payDate, paymentMethod, selectedSubscription, enrollment, totalCost);
	...
	/*	if (selectedAthlete == null) {
	        showAlert(Alert.AlertType.ERROR, "Error", "Selected athlete is null.");
	        return;
	    }*/
	    ...
	    if (!selectedAthlete.getIsProfessional()) {
	        ...
	        extraDiscountRate = 0.0;
	    }

	    if (selectedAthlete.getExperience() >= 1 && selectedAthlete.getExperience() <= 2) {
	        discountRate = 0.1;
	    } else if (selectedAthlete.getExperience() <= 4) {
	        discountRate = 0.2;
	    } else if (selectedAthlete.getExperience() <= 5) {
	        discountRate = 0.3;
	    }
