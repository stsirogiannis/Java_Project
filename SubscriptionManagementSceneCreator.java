...
public class SubscriptionManagementSceneCreator extends SceneCreator implements EventHandler<MouseEvent>  {
  ...
  //  private Athlete selectedAthlete;
	...
	//private Labeled totalCostField; 
	...
	public SubscriptionManagementSceneCreator(double width, double height) {
       ...
       /* TableColumn<Subscription, Double> totalCostColumn= new TableColumn<>("Συνολικό Ποσό");
	       ...
	       if (!selectedAthlete.getIsProfessional()) {
	        ...
	        createPayment(payCode, payDate, paymentMethod, selectedSubscription, selectedEnrollment, totalCost);
	           /* double extraDiscountRate=0.0;
	            ...
	            if (selectedAthlete != null && !selectedAthlete.getIsProfessional()) {
	                extraDiscountRate = 0.2;
	            }

	            // Assuming you want to find the specific athlete from a list
	            List<Athlete> ath = AthleteManagementSceneCreator.getAthleteList();
	            Athlete athlete = null;

	            // Example logic to find the specific athlete (assuming some criteria)
	            for (Athlete a : ath) {
	                if (a.equals(selectedAthlete)) { // Assuming proper equals method is overridden
	                    athlete = a;
	                    break;
	                }
	            }

	            // Ensure athlete is found
	            if (athlete != null) {
	                int experience = athlete.getExperience();

	                // Apply discount based on experience
	                if (experience >= 1 && experience <= 2) {
	                    discountRate = 0.1;
	                } else if (experience > 2 && experience <= 4) {
	                    discountRate = 0.2;
		...
		//    calculateTotalCost();
		 ...
		 if (event.getSource() == subscriptionTableView) {
		       ...
		       /*     // Find the associated payment
		            Payment associatedPayment = null;
		            for (Payment payment : paymentList) {
		                if (payment.getSubscription().equals(selectedSubscription)) {
		            ...
		            if (associatedPayment != null) {
		                totalCostField.setText(String.valueOf(associatedPayment.getTotalCost()));    
		            }*/
		        }
		 }      
