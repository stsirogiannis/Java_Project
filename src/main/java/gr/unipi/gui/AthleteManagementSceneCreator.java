...
public class AthleteManagementSceneCreator extends SceneCreator implements EventHandler<MouseEvent> {
    ...
    // Initialize fields
    private int userCount=0; //unique user code
    ...
    public AthleteManagementSceneCreator(double width, double height) {
        ...
        // Customize enrollmentTableView: Create Column, create Cell and set it to Column. Add Column to TableView
    ...
    public void handle(MouseEvent event) {
        ...
        if (event.getSource() == newAthleteBtn) {
            ...
            // FIX: guard against no professional-status selected
            Toggle selectedProfessional = professionalToggleGroup.getSelectedToggle();
            if (selectedProfessional == null) {
                Alert alertType = new Alert(Alert.AlertType.ERROR);
                alertType.setTitle("Μη έγκυρη επιλογή");
                alertType.setContentText("Παρακαλώ επιλέξτε αν ο αθλητής είναι επαγγελματίας.");
                alertType.show();
                userCount--;
                return;
            }
            boolean isProfessional = ((RadioButton) selectedProfessional).getText().equals("Ναι");

            //check if the experience is 1-5
            try {
            ...
            //sync the tableview with the new values and clear the textFields
            tableSync();
            clearTextFields();
        ...
        if (event.getSource() == updateAthleteBtn) {
            ...
            boolean isProfessional = ((RadioButton) selectedProfessional).getText().equals("Ναι"); // FIX: was .equals("Yes")
    ...
    private void showEnrollmentPayWindow(Athlete selectedAthlete) {
        ...
        //crete new window for the purposes of the payment functionality
        ...
        payButton.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent e) {
                ...
                //user enters date and method of payment
                String dateString = dateTextField.getText();
                String method = paymentMethodChoice.getValue();

                //check if the date and the method of payment are empty
    ...
    // create a new Enrollment object and add it to enrollmentList
    ...
    //get the athlete list from the Athlete class
