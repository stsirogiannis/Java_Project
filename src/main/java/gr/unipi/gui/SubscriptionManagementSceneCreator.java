...
public class SubscriptionManagementSceneCreator extends SceneCreator implements EventHandler<MouseEvent>  {
    ...
    public void handle(MouseEvent event) {
        ...
        if (event.getSource() == newSubBtn) {
            ...
            // FIX: guard against no professional-access selection
            Toggle selectedProfAccess = professionalAccessOnlyToggleGroup.getSelectedToggle();
            if (selectedProfAccess == null) {
                Alert alertType = new Alert(Alert.AlertType.ERROR);
                alertType.setTitle("Μη έγκυρη επιλογή");
                alertType.setContentText("Παρακαλώ επιλέξτε αν το άθλημα απαιτεί πρόσβαση μόνο σε επαγγελματίες.");
                alertType.show();
                subCount--;
                TPCount--;
                return;
            }
            boolean professionalAccess = ((RadioButton) selectedProfAccess).getText().equals("Ναι");
            ...
            // FIX: guard against no coach-gender selection
            Toggle selectedCoachGender = coachToggleGroup.getSelectedToggle();
            if (selectedCoachGender == null) {
                Alert alertType = new Alert(Alert.AlertType.ERROR);
                alertType.setTitle("Μη έγκυρη επιλογή");
                alertType.setContentText("Παρακαλώ επιλέξτε φύλο προπονητή.");
                alertType.show();
                subCount--;
                TPCount--;
                return;
            }
            char genderCoach = ((RadioButton) selectedCoachGender).getText().charAt(0);
            ...
            // FIX: guard against no weekly-reservation selection
            Toggle selectedWeekly = weeklyReservationToggleGroup.getSelectedToggle();
            if (selectedWeekly == null) {
                Alert alertType = new Alert(Alert.AlertType.ERROR);
                alertType.setTitle("Μη έγκυρη επιλογή");
                alertType.setContentText("Παρακαλώ επιλέξτε εβδομαδιαία κράτηση.");
                alertType.show();
                subCount--;
                TPCount--;
                return;
            }
            boolean weeklyReservation = ((RadioButton) selectedWeekly).getText().equals("Ναι");

            // FIX: guard against no participants'-gender selection
            Toggle selectedGender = genderToggleGroup.getSelectedToggle();
            if (selectedGender == null) {
                Alert alertType = new Alert(Alert.AlertType.ERROR);
                alertType.setTitle("Μη έγκυρη επιλογή");
                alertType.setContentText("Παρακαλώ επιλέξτε φύλο συμμετεχόντων.");
                alertType.show();
                subCount--;
                TPCount--;
                return;
            }
            char gender = ((RadioButton) selectedGender).getText().charAt(0);
    ...
    private void createFacility(String facilityName, int maxCapacity) {
        this.facility = new Facility(facilityName, maxCapacity); // FIX: assign to field, not a local var
        facilityList.add(this.facility);
    }

    // create a new Sport object and add it to sportList
    private void createSport(String sportName, boolean professionalAccess) {
        this.sport = new Sport(sportName, professionalAccess); // FIX: assign to field, not a local var
        sportList.add(this.sport);
    }
    ...
    public void createCoach( int coachCount, String nameCoach, String surnameCoach, char genderCoach, String birthDateCoach, String contactInfoCoach,Sport sport, int degrees) {
        this.coach = new Coach(coachCount, nameCoach, surnameCoach, genderCoach, birthDateCoach, contactInfoCoach, sport, degrees); // FIX: assign to field, not a local var
        coachList.add(this.coach);
    }
